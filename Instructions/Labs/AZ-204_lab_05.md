...
#### Tarefa 3: Abrir o Cloud Shell e editor

1. No portal do Azure, selecione o ícone **Cloud Shell** ![Ícone do Cloud Shell](./media/az204_lab_CloudShell.png) para abrir uma nova sessão do PowerShell. Se o Cloud Shell tiver como padrão uma sessão do PowerShell, selecione **PowerShell** e, então, no menu suspenso, selecione **Bash**.
...
1. No prompt de comando do **Cloud Shell**no portal, execute o seguinte comando para mover do diretório raiz para o diretório **\~/clouddrive**:

    ```
    cd ~/clouddrive
    ```

1. Execute o seguinte comando para criar um novo diretório chamado **ipcheck** no diretório **\~/clouddrive**:

    ```
    mkdir ipcheck
    ```

1. Execute o seguinte comando para alterar o diretório ativo de **\~/clouddrive** para **\~/clouddrive/ipcheck**:

    ```
    cd ~/clouddrive/ipcheck
    ```

1. Execute o comando a seguir para criar um novo aplicativo de console do .NET no diretório atual:

    ```
    dotnet new console --output . --name ipcheck --framework net7.0
    ```

1. Execute o comando a seguir para criar um novo arquivo no diretório **\~/clouddrive/ipcheck** chamado **Dockerfile**:

    ```
    touch Dockerfile
    ```

1. Execute o seguinte comando para abrir o editor gráfico integrado no contexto do diretório atual:
...
1. No editor gráfico, no painel **FILES**, selecione o arquivo **Program.cs** para abri-lo no editor.
...
1. Salve o arquivo **Program.cs** usando o menu no editor gráfico ou o atalho de teclado Ctrl+S.  Não feche o editor gráfico.

1. Voltando para o prompt de comando, execute o comando a seguir para executar o aplicativo:

    ```
    dotnet run
    ```

1. Examine os resultados da execução. Pelo menos um endereço IP deve estar listado para a instância do Cloud Shell.

1. No editor gráfico, no painel **FILES** do editor, selecione o arquivo **Dockerfile** para abri-lo no editor.
...
1. Salve o arquivo **Dockerfile** usando o menu no editor gráfico ou o atalho de teclado Ctrl+S.

1. Deixe o Cloud Shell aberto para a próxima tarefa.
...
1. Execute o seguinte comando para alterar o diretório ativo de **\~/** para **\~/clouddrive/ipcheck**:
...
1. Na seção **Repositórios**, selecione o repositório de imagem de contêiner **ipcheck**.

1. Na folha **Repositório**, selecione o menu de reticências associado com a entrada de marca **mais recente** e, então, selecione **Executar a instância**.
