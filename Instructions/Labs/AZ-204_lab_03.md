...
Por fim, para garantir conformidade com os regulamentos da empresa, você implementará o acesso seguro gerando assinaturas de acesso compartilhado e políticas de ciclo de vida de dados. Você também testará o acesso ao conteúdo por meio de site estático.
...
1. Na folha **Criar uma conta de armazenamento**, na guia **Básico**, realize as ações a seguir:
...
1. Na guia **Avançado**, verifique se a opção **Permitir habilitar o acesso anônimo em contêineres individuais** está marcada. Marque a configuração se não estiver habilitada.
...
1. Examine qualquer uma das chaves e, em seguida, copie o valor de uma das caixas **Chave** para a área de transferência.
...
1. No terminal, execute o seguinte comando para importar a versão 12.12.0 do **Azure.Storage.Blobs** do NuGet:

    ```
    dotnet add package Azure.Storage.Blobs --version 12.12.0
    ```

    > **Observação**: O comando **dotnet add package** adicionará o pacote **Azure.Storage.Blobs** do NuGet. Para obter mais informações, consulte [Azure.Storage.Blobs](https://www.nuget.org/packages/Azure.Storage.Blobs/12.12.0).
